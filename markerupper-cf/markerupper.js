(function () {
  ...
  function menuClicked (e) {
   ...
   // Toggle the menu button that was clicked
    ...
    if (type === 'backColor'){
          document.execCommand(type, false, highlightColor) // toggle the type of whatever is selected
    }
    else {
    ...
    for (key in keys) {
      if (Right_Click.options.modifiers[key]) {
        if (key === 'justify') {
          ...
          highlightColor = Right_Click.options.modifiers[key]
        } else { arr.push(key) }
      }
    }
